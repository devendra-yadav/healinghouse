...
if [ "a$HEALING_HOUSE_DB_PASSWORD" == "a" ]; then
  ...
  if [ "a$pid" == "a" ]; then
    nohup java -Xms$APP_INIT_JAVA_HEAP -Xmx$APP_MAX_JAVA_HEAP -DHEALING_HOUSE_DB_PASSWORD=$HEALING_HOUSE_DB_PASSWORD -Dlogs_dir=$logs_dir -Dlogging.config=$logback_config -jar $app_home/lib/healinghouse.jar --spring.profiles.active=test&
    PID=$!
