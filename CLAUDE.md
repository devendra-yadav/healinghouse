...
- Sub-folders mirror domain: `patients/`, `therapists/`, `services/`, `products/`, `appointments/`, `tags/`, `reports/`, `combos/`, `package-templates/`
...
PackageTemplate ──< PackageTemplateServiceItem >── ClinicService
                ──< PackageTemplateProductItem >── Product

Patient ──< PatientPackage >── PackageTemplate (sourceTemplate, nullable — informational only)
PatientPackage ──< PatientPackageServiceItem >── ClinicService
               ──< PatientPackageProductItem >── Product
               ──< PackageTransaction
...
User (app_user) ── AppRole (OWNER/ADMIN/RECEPTIONIST/THERAPIST/THERAPIST_PLUS, enum column, not a table)
User >── Therapist (nullable therapist_id — set iff role == THERAPIST or THERAPIST_PLUS, at most one User per Therapist)
RolePermission (role, module, action, granted) — no FK to User; keyed by the AppRole enum itself

ExpenseCategory ──< Expense (categorizes)
ExpenseCategory ──< RecurringExpenseTemplate (categorizes)
RecurringExpenseTemplate ──< Expense (nullable sourceTemplate — auto-generated vs. manual)
Therapist ──< Expense (nullable — set iff category is the payout category)
...
- `util/ProportionalAllocator` — the sort-ascending/last-line-absorbs-remainder proportional-split algorithm, extracted out of `AppointmentService` (was a private `distributeAmount` method) so `PackageService.sellPackage` could reuse the exact same fair-split math to divide a package sale's `totalPrice` across its items. `AppointmentService.applyComboDiscount`/`distributeDiscount` now delegate to it too — behavior-preserving refactor, still the single allocator behind combo discounts, whole-appointment discounts, and package `priceAllocated` splits.
- `PatientPackage` (like `PatientWallet`) has `@Version`, but the mutated field during consumption (`sessionsUsed`) lives on a *child* item entity (`PatientPackageServiceItem`/`ProductItem`), not on `PatientPackage` itself — a plain `save()` of the parent wouldn't dirty it or check/bump the version. `PackageService.lockAndPersist` uses `entityManager.lock(pkg, LockModeType.OPTIMISTIC_FORCE_INCREMENT)` before `flush()` to force the version check anyway. See the Packages business rule below.
...
- `GET /combos/search` browses the full active/selectable combo list on a blank `q` (fired on modal open) and name-filters on a non-blank one — both paths are paginated identically, wrapped in `ComboSearchResultDTO` (page size defaults to `comboMaxSuggestions`, clamped via `PaginationUtil`). `ComboService.searchSelectable(query, pageable)` slices the already-`isSelectable`-filtered in-memory list (that filter can't be pushed into the DB query) rather than paginating at the repository level — acceptable for a clinic-sized combo catalog. Distinct from the same-class `search(query, pageable)` overload the `/combos` list page uses, which has different active/inactive semantics — don't conflate the two. The modal's pager (`comboPickerPager`, Page X of Y + prev/next) mirrors the compact pager style used elsewhere (e.g. the patient detail page's Packages card).
...
- `AppointmentServiceLine.packageServiceItem`/`AppointmentProductLine.packageProductItem` (nullable FKs, mirrors `appointmentCombo`) mark a line as package-covered; `Appointment.packageAmountApplied` (mirrors `walletAmountApplied`) is always exactly the sum of covered lines' own raw values (no partial coverage, no discount interaction — package coverage is a payment source, not a discount) and folds additively into `amountPaid`.
- **The clear-and-rebuild reconciliation** (`AppointmentService.updateAppointment`, `reconcilePackageDelta`) is the trickiest part of the feature: because a single `PatientPackageServiceItem`/`ProductItem` can legitimately back more than one line in the same appointment (staff clicking "Add" twice for a service with 2+ pooled sessions), old-vs-new lines are compared by a **multiset (occurrence-count) diff per item id**, not a set diff — `delta = newCount − oldCount` per item, `delta > 0` consumes that many additional sessions, `delta < 0` reverses that many, `delta == 0` is a no-op (critically, does *not* reverse-then-reapply, which would risk a stale double-decrement or landing on a different FIFO-picked item). Runs after `saveWithConflictCheck` succeeds, mirroring wallet's debit-last ordering so an insufficient-sessions failure rolls back the whole update.
- Cancelling/no-showing an appointment reverses every package-covered line via `reverseAllPackageConsumption` (mirrors `reverseFullWalletIfAny`) — one reverse call per line, no multiset math needed since every attached line is reversed outright, not diffed against a resubmission.
- `PackageService.consumeServiceItem`/`consumeProductItem`/`reverseServiceItem`/`reverseProductItem` each write one `PackageTransaction` (`USAGE`/`REVERSAL`) linked to the specific item + the appointment, and use `entityManager.lock(..., OPTIMISTIC_FORCE_INCREMENT)` (see the `PatientPackage`/`@Version` design decision above) to guard concurrent consumption.
- Expiry is optional per `PatientPackage`, enforced **lazily** wherever eligibility is checked (`getPooledAvailability`) — no background job; an `ACTIVE` package past its `expiryDate` flips to `EXPIRED` the next time it's touched. Unused value on an expired/cancelled package is never auto-forfeited — refund (`POST /patients/{id}/packages/{packageId}/refund`) is always a manual staff action, computed as `Σ priceAllocated × sessionsRemaining/sessionsTotal` (transient, `getBalanceDue()`-style), and never touches `sessionsUsed`.
- Commission, stock decrement, and double-booking conflict detection are all unaffected — package-covered lines are ordinary `AppointmentServiceLine`/`AppointmentProductLine` rows; commission queries key off `priceAtTime`/`lineTotal`/`therapist`/tags only, never the package FK. Revenue recognition is likewise unaffected: a package sale (`PURCHASE`/`REFUND` `PackageTransaction`) is a liability, never revenue — revenue is recognized off `grandTotal` at `COMPLETED`, identical timing regardless of payment source.
- See `requirements/Packages_Requirements_v1.md` for the full spec.
...
**Expenses & Profit/Loss** — the clinic's cost side, entirely decoupled from the revenue-side catalog (`Product`/`ClinicService`/`Tag`), plus a fifth role trusted to record costs without a full Admin promotion:
- `ExpenseCategory` (own soft-delete lifecycle, `active` toggle + guarded permanent-delete — same pattern as `Combo`/`ClinicService`/`Product`) is a completely independent master list, never linked to `Product`/`ClinicService`/`Tag` — an expense for "acupuncture needles" is a cost row only, never selectable on an appointment, never billed to a patient, never touches commission. `restrictedVisibility` (bool) hides every `Expense` under that category from `THERAPIST_PLUS`. `SecuritySeeder.seedExpenseCategories()` seeds a starter list (Raw Materials, Equipment, Maintenance, Utilities, Rent, **Salaries**, **Commission** — both `restrictedVisibility=true` — and Other), only-if-empty, alongside its role-permission seeding (real operational master data needed in every environment including prod, unlike `DataSeeder`'s dev/test-only sample data). `SecuritySeeder.splitSalariesAndCommissionCategory()` is a one-time idempotent backfill (mirrors `OwnerFlagBackfill`'s pattern) for databases seeded before this split — it renames a pre-existing "Salaries & Commission" row in place to "Salaries" (same id, so historical `Expense.category` FKs stay valid) and inserts a new sibling "Commission" category.
- `Expense` (`ACTIVE`/`VOIDED` via `ExpenseStatus`, `@Version` optimistic lock like `Appointment`/`PatientWallet`) is never hard-deleted — "delete" sets `status = VOIDED`, which excludes it from every report/list by default and makes it **immutable** (cannot be edited or un-voided); correcting a mistake means voiding the wrong entry and recording a fresh, correct one, preserving a clean audit trail via `recordedBy`/`createdAt`. There is no dedicated therapist link or "payout category" flag — a salary/commission payout is simply an ordinary `Expense` under the "Salaries"/"Commission" category, with the payee noted in the free-text `label` field; this is a **separate, parallel record** of actual money paid out and never modifies `CommissionCalculator`'s own read-only computation.
- `RecurringExpenseTemplate` (`MONTHLY`/`QUARTERLY`/`YEARLY` via `RecurrenceFrequency`, `nextDueDate` cursor, optional `endDate`) auto-generates a normal, freely-editable `ACTIVE` `Expense` each period via **`RecurringExpenseScheduler`** — the first scheduled/background job in this codebase (`@Scheduled(cron = "0 0 1 * * *")`, requires `@EnableScheduling` on `HealinghouseApplication`). No explicit `zone=` is needed on the cron expression since the JVM default timezone is already globally forced to Asia/Kolkata (see the timezone bullet below) — it fires at 01:00 IST wall-clock, not UTC; don't "fix" this by adding a zone attribute without reconsidering that global forcing. `nextDueDate` advances by exactly one interval per generation (idempotent — no double-generation on re-run or app-restart replay); once the advanced date passes `endDate` the template auto-deactivates, mirroring the "auto-deactivate when left empty" pattern combos already use. A generated expense that differs from `defaultAmount` (e.g. a variable utility bill) is simply edited afterward like any other expense — same manual "Generate Now" trigger (`RecurringExpenseTemplateService.generateDueExpenses`) backs both the scheduler and a staff-facing button, with the identical one-generation-per-due-date guarantee.
- **`THERAPIST_PLUS`** — a fifth `AppRole`, identical to `THERAPIST` everywhere else (same `User.therapist`-link validation, same "own appointments/earnings only" scoping) plus: full `EXPENSES` CRUD (view/create/edit/void, not permanent-delete), and full catalog CRUD (view/create/edit/deactivate, not permanent-delete/`APPROVE`) on `SERVICES`/`PRODUCTS`/`COMBOS`/`PACKAGE_TEMPLATES` — a therapist trusted enough to record clinic expenses is also trusted to maintain the day-to-day catalog without a full Admin promotion (`requirements/Expenses_Requirements_v1.md` §5.5 addendum, confirmed by the clinic owner; `requirements/Bug_Report_v6.md` Finding 1 initially flagged the resulting seed change as an unreviewed escalation, closed not-a-bug once the addendum was written). It is **never** granted `REPORTS_PROFIT_LOSS` or `REPORTS_REVENUE` — no Net Revenue/Net Profit visibility. Independently of that module/action gate, `ExpenseService` inline-filters out every `Expense` whose `category.restrictedVisibility = true` for a `THERAPIST_PLUS` caller (attribute-based scoping, not identity-based — can't reuse `PermissionAspect` or the existing `enforceOwnXxxForTherapist` helpers, since those key off *who* the caller is, not a joined entity's flag); a direct-URL lookup of a restricted expense's id throws `EntityNotFoundException` (existence-masking), not `AccessDeniedException`, so a `THERAPIST_PLUS` caller can't even confirm the row exists. `RECEPTIONIST` and plain `THERAPIST` get no grants on `EXPENSE_CATEGORIES`/`EXPENSES`/`REPORTS_PROFIT_LOSS` at all.
- New **Profit & Loss** report (`GET /reports/profit-loss`, `ProfitLossReportAggregator`, Owner/Admin only) — Net Revenue (Σ `grandTotal`, `COMPLETED` appointments — same basis the Actual Revenue report already established, so the two numbers reconcile for the same date range) minus Total Expenses (Σ `ACTIVE` `Expense.amount` in range) = Net Profit, with a by-category breakdown and a **month-over-month** trend (`ProfitLossTrendPointDTO` — bucketed by calendar month, not daily; `requirements/Bug_Report_v6.md` Finding 8 caught and fixed an earlier daily-bucketed version that didn't match the spec/DTO). Follows the same CSV/PDF export triad, date-range-defaulting, and branded-PDF conventions every other report uses. Two Dashboard KPI tiles (`totalExpensesThisMonth`/`netProfitThisMonth` on `DashboardKpiDTO`) read the same aggregator scoped to the current calendar month — `DashboardService` deliberately calls the cheap `totalExpenses`/`netProfit`-only path, not the full report (category breakdown + trend), to avoid unnecessary work on every dashboard load.
- Reporting/commission isolation is otherwise total: `CommissionCalculator` never reads `Expense`/`ExpenseCategory` regardless of whether a payout was separately logged as an expense; stock decrement, double-booking conflict detection, discounts, wallet, and package consumption are all unrelated — Expenses touch no `Appointment`-side logic at all.
- See `requirements/Expenses_Requirements_v1.md` for the full spec and `requirements/Bug_Report_v6.md` for the post-implementation review (all findings fixed or closed not-a-bug).
...
- Five fixed roles (`AppRole`: OWNER, ADMIN, RECEPTIONIST, THERAPIST, THERAPIST_PLUS — the last added by the Expenses feature, see the Expenses & Profit/Loss business rule below), one `User` per login (`BCryptPasswordEncoder`, `app_user` table). A THERAPIST/THERAPIST_PLUS-role `User` carries a nullable `therapist` FK (validated: must be set iff role is THERAPIST or THERAPIST_PLUS, at most one `User` per `Therapist`) — that link is what all "own data only" scoping resolves against. `THERAPIST_PLUS` is a THERAPIST for every scoping purpose, plus additional Expenses/catalog grants — it is not a separate scoping tier.
...
- Expenses & Profit/Loss — one-off and recurring clinic expenses against an admin-managed category list, a new Profit & Loss report, a new `THERAPIST_PLUS` role, and the app's first scheduled background job — see the dedicated business rule above and `requirements/Expenses_Requirements_v1.md`. New: `ExpenseCategory`/`Expense`/`RecurringExpenseTemplate` entities, `ExpenseCategoryService`/`ExpenseService`/`RecurringExpenseTemplateService`/`RecurringExpenseScheduler`, `ExpenseCategoryController`/`ExpenseController`/`RecurringExpenseTemplateController`, `ProfitLossReportAggregator` + `ReportController`'s `/reports/profit-loss` (bringing the report count to seven), two new Dashboard KPI tiles, `templates/expense-categories/`, `templates/expenses/`, `templates/reports/profit-loss.html`.
- Full bug-report findings/fixes trail: `requirements/Bug_Report_v1.md` through `Bug_Report_v6.md` (all findings through v5 marked FIXED; v6 — the post-Expenses-implementation review — found 12 issues at Medium/High severity, all now FIXED or closed not-a-bug, plus 11 Low-severity notes). `mvnw test` 102/102 passing as of 2026-07-27 (up from 73/73 on 2026-07-19 — the growth is the Expenses/Profit-Loss/THERAPIST_PLUS test coverage).
