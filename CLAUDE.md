...
- `ExpenseCategory` (own soft-delete lifecycle, `active` toggle + guarded permanent-delete — same pattern as `Combo`/`ClinicService`/`Product`) is a completely independent master list, never linked to `Product`/`ClinicService`/`Tag` — an expense for "acupuncture needles" is a cost row only, never selectable on an appointment, never billed to a patient, never touches commission. `restrictedVisibility` (bool) hides every `Expense` under that category from `THERAPIST_PLUS`; `payoutCategory` (bool) marks the category as a therapist-salary/commission payout category (drives the therapist picker + server-side required-therapist validation on the Expense form — keyed off this flag, not a hardcoded category name match, precisely so renaming the category can't silently break the validation). `SecuritySeeder.seedExpenseCategories()` seeds a starter list (Raw Materials, Equipment, Maintenance, Utilities, Rent, **Salaries & Commission** — `restrictedVisibility=true`, `payoutCategory=true` — and Other), only-if-empty, alongside its role-permission seeding (real operational master data needed in every environment including prod, unlike `DataSeeder`'s dev/test-only sample data).
- `Expense` (`ACTIVE`/`VOIDED` via `ExpenseStatus`, `@Version` optimistic lock like `Appointment`/`PatientWallet`) is never hard-deleted — "delete" sets `status = VOIDED`, which excludes it from every report/list by default and makes it **immutable** (cannot be edited or un-voided); correcting a mistake means voiding the wrong entry and recording a fresh, correct one, preserving a clean audit trail via `recordedBy`/`createdAt`. `Expense.therapist` is set only when the category is the payout category (§ above) — recording a payout this way is a **separate, parallel record** of actual money paid out and never modifies `CommissionCalculator`'s own read-only computation; the Expense form shows that computed `totalVariablePay` figure only as a non-binding reference hint.
