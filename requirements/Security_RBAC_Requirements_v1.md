...
| Patient Packages (sell / refund) | V,C,E,A | V,C,E,A | V,C,A *(sell)* | V,C,A *(sell + refund)* |
| Wallet (top-up / refund) | V,C,A | V,C,A | V,C | V,C,A *(top-up + refund)* |
...
- **Data scoping for THERAPIST** (beyond the module/action gate — row-level filtering):
  ...
  - *Wallet / Patient Packages:* THERAPIST can top up a patient's wallet, sell a package, **and refund either** (`CREATE` + `APPROVE`, beyond RECEPTIONIST's `CREATE`-only grant), since front-desk-style patient payment tasks — including corrections/refunds — are routinely handled by therapists at this clinic too. `WalletController.refund`/`PackageController.refund` have no additional ownership check beyond the module/action gate — same reasoning as the `VIEW` endpoints below. `GET /patients/{id}/wallet/balance` and `GET /patients/{id}/packages/available` (the JSON endpoints backing the wallet balance card and the appointment form's "Already Paid" section) are gated only by the `WALLET`/`PATIENT_PACKAGES` `VIEW` grant, with no additional "already has an appointment with this patient" ownership check — matching the *Patients* row above (full unscoped read access) and `PatientController`'s detail page, which already renders wallet/package data for any patient with the same no-ownership-check reasoning. An ownership check here would only break the legitimate case of a therapist's *first* appointment with a patient (no prior appointment yet to satisfy the check) while adding no real protection, since therapists can already book a new appointment for any patient unrestricted.
